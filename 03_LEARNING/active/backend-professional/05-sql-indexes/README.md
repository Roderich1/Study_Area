...
Leer planes de ejecución y justificar índices según selectividad, cardinalidad y coste.

## Por qué importa en producción

comparar una consulta lenta antes y después de un índice usando PostgreSQL real. Un backend profesional debe hacer explícitos sus límites, fallos y costes; una implementación que funciona una vez no demuestra comportamiento seguro bajo carga, concurrencia o evolución.
...
JOIN, GROUP BY, subqueries, CTE, EXPLAIN, EXPLAIN ANALYZE, B-tree, índices compuestos, parciales y covering, selectivity, cardinality y trade-offs.
...
Implementar o instrumentar la fase de queries, EXPLAIN ANALYZE e índices del escenario de [LAB-BACKEND-004-postgresql-model-and-query-plan](../../../../04_LABS/backend/LAB_INDEX.md#lab-backend-004-postgresql-model-and-query-plan) dentro de production-api-lab. Romperlo deliberadamente cuando sea seguro, observar la evidencia y registrar resultado esperado y real por separado.
...
**LAB-BACKEND-004-postgresql-model-and-query-plan** — cargar un dataset, leer EXPLAIN ANALYZE y comparar una consulta antes y después de un índice usando PostgreSQL real. La planificación y el contrato del laboratorio están en [04_LABS/backend/LAB_INDEX.md](../../../../04_LABS/backend/LAB_INDEX.md).
...
[PostgreSQL EXPLAIN](https://www.postgresql.org/docs/current/sql-explain.html) y [indexes](https://www.postgresql.org/docs/current/indexes.html). Mantener la lista curada en [09_RESOURCES/backend-resources.md](../../../../09_RESOURCES/backend-resources.md).
...
Después de estudiar y verificar, podrían promoverse explicaciones refinadas sobre JOIN, GROUP BY, subqueries, CTE, EXPLAIN, EXPLAIN ANALYZE, B-tree, índices compuestos, parciales y covering, selectivity, cardinality y trade-offs, separadas de los apuntes crudos. Este módulo no afirma que ese conocimiento ya esté consolidado.

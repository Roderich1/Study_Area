...
Elegir métodos, códigos y headers HTTP que expresen correctamente la semántica de una API.

## Por qué importa en producción

diseñar y probar solicitudes condicionales, comparando PUT/PATCH, 401/403 y 409/422. Un backend profesional debe hacer explícitos sus límites, fallos y costes; una implementación que funciona una vez no demuestra comportamiento seguro bajo carga, concurrencia o evolución.
...
GET, POST, PUT, PATCH, DELETE, HEAD, OPTIONS, safe, idempotency, status codes, headers, content negotiation, caching, ETag, If-Match, If-None-Match y conditional requests.
...
Implementar o instrumentar el escenario de [LAB-BACKEND-002-http-conditional-requests](../../../../04_LABS/backend/LAB_INDEX.md#lab-backend-002-http-conditional-requests) dentro de production-api-lab. Romperlo deliberadamente cuando sea seguro, observar la evidencia y registrar resultado esperado y real por separado.
...
**LAB-BACKEND-002-http-conditional-requests** — diseñar y probar solicitudes condicionales, comparando PUT/PATCH, 401/403 y 409/422. La planificación y el contrato del laboratorio están en [04_LABS/backend/LAB_INDEX.md](../../../../04_LABS/backend/LAB_INDEX.md).
...
[RFC 9110](https://www.rfc-editor.org/rfc/rfc9110) y [MDN conditional requests](https://developer.mozilla.org/en-US/docs/Web/HTTP/Conditional_requests). Mantener la lista curada en [09_RESOURCES/backend-resources.md](../../../../09_RESOURCES/backend-resources.md).
...
Después de estudiar y verificar, podrían promoverse explicaciones refinadas sobre GET, POST, PUT, PATCH, DELETE, HEAD, OPTIONS, safe, idempotency, status codes, headers, content negotiation, caching, ETag, If-Match, If-None-Match y conditional requests, separadas de los apuntes crudos. Este módulo no afirma que ese conocimiento ya esté consolidado.

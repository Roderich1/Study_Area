...
Diseñar operaciones resistentes a timeouts, reintentos y duplicación de solicitudes.

## Por qué importa en producción

hacer que POST /payments produzca un único efecto lógico para una misma Idempotency-Key. Un backend profesional debe hacer explícitos sus límites, fallos y costes; una implementación que funciona una vez no demuestra comportamiento seguro bajo carga, concurrencia o evolución.
...
failure modes, timeouts, retries, exponential backoff, jitter, idempotency keys, duplicate requests, circuit breakers y dependencias externas.
...
Implementar o instrumentar el escenario de [LAB-BACKEND-009-idempotency-key](../../../../04_LABS/backend/LAB_INDEX.md#lab-backend-009-idempotency-key) dentro de production-api-lab. Romperlo deliberadamente cuando sea seguro, observar la evidencia y registrar resultado esperado y real por separado.
...
**LAB-BACKEND-009-idempotency-key** — hacer que POST /payments produzca un único efecto lógico para una misma Idempotency-Key. La planificación y el contrato del laboratorio están en [04_LABS/backend/LAB_INDEX.md](../../../../04_LABS/backend/LAB_INDEX.md).
...
[HTTP Semantics](https://www.rfc-editor.org/rfc/rfc9110) y [Spring Retry](https://docs.spring.io/spring-retry/reference/index.html). Mantener la lista curada en [09_RESOURCES/backend-resources.md](../../../../09_RESOURCES/backend-resources.md).
...
Después de estudiar y verificar, podrían promoverse explicaciones refinadas sobre failure modes, timeouts, retries, exponential backoff, jitter, idempotency keys, duplicate requests, circuit breakers y dependencias externas, separadas de los apuntes crudos. Este módulo no afirma que ese conocimiento ya esté consolidado.
